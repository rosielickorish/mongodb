...
function ItemDAO(database) {
    ...
    this.getCategories = function(callback) {
        ...
        */
        ...
        var query_all = [{$project:{_id:1, category:1, title:1}}, {$group:{_id:"All", num:{$sum:1}}}, {$sort:{_id:1}} ];

        var cursor = this.db.collection('item').aggregate(query)
        var cursor_all = this.db.collection('item').aggregate(query_all)
        cursor.sort({"category":1});
        var categories = []
        cursor_all.toArray(function(err,docs){
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
        ...
        if (category == "All"){
          ...
          var cursor = this.db.collection('item').find({"category": category})
        }

        cursor.sort({"_id":1}).limit(itemsPerPage).skip(itemsPerPage*page);
        var page_Items = [];
        cursor.toArray(function(err,docs){
          docs.forEach(function(doc){
              page_Items.push(doc)
    ...
    this.searchItems = function(query, page, itemsPerPage, callback) {
         ...
         * this method, create a SINGLE text index on title:1, slogan:1, description:1 and
    ...
    this.addReview = function(itemId, comment, name, stars, callback) {
        ...
        var cursor = this.db.collection('item').update({"_id":itemId}, {$set:{reviewDoc}});
